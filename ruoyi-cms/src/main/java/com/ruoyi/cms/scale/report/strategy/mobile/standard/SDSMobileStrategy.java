...
public class SDSMobileStrategy implements ITemplateStrategy {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SDSMobileStrategy.class);
    ...
    public String getTemplate(Long contextId, String deviceType, LbsResultsVo lbsResults) {
        try {
            ...
            try {
                logger.info("Processing template: mobile/standard/sds");
                logger.info("Context variables: {}", context.getVariableNames());

                // 添加模板文件检查
                ClassLoader classLoader = getClass().getClassLoader();
                String templatePath = "templates/mobile/standard/sds.html";
                java.net.URL resource = classLoader.getResource(templatePath);
                logger.info("Template resource URL: {}", resource);

                // 尝试处理模板
                String result = templateEngine.process("mobile/standard/sds", context);
                ...
                logger.error("Unexpected error processing template: ", e);
    ...
    private boolean hasRequiredFields(Map<String, Object> resultMap) {
        ...
        boolean hasDepressionLevel = resultMap.containsKey("depressionLevel") && resultMap.get("depressionLevel") != null;

        // 只要包含标准分和抑郁程度就认为数据有效
        return hasStandardScore && hasDepressionLevel;
    }
    ...
    private void ensureRequiredFields(Map<String, Object> resultMap) {
        ...
        // 抑郁程度
        if (!resultMap.containsKey("depressionLevel") || resultMap.get("depressionLevel") == null) {
            resultMap.put("depressionLevel", "无抑郁");
        }
    ...
    private Context prepareTemplateContext(Map<String, Object> resultMap) {
        ...
        String depressionLevel = (String) resultMap.get("depressionLevel");
        ...
        // 获取抑郁程度的颜色配置
        Map<String, String> levelColors = getLevelColors(depressionLevel);
        ...
        context.setVariable("depressionLevel", depressionLevel);
        ...
        // 抑郁程度描述
        context.setVariable("levelDescription", getLevelDescription(depressionLevel, standardScore));
    ...
    private List<String> prepareSuggestionList(List<String> suggestionList, String suggestions) {
        ...
        if (result.isEmpty()) {
            ...
            result.add("适当进行户外活动和体育锻炼");
            result.add("与亲友保持良好沟通");
     ...
     * 根据抑郁程度获取颜色配置
     */
    private Map<String, String> getLevelColors(String depressionLevel) {
     ...
     * 获取抑郁程度描述
     */
    private String getLevelDescription(String depressionLevel, int score) {
        switch (depressionLevel) {
            case "无抑郁":
                return "您的情绪状态良好，处于正常范围。标准分" + score + "分，表明没有明显的抑郁症状。";
            case "轻度抑郁":
                return "您的抑郁症状处于轻度水平，标准分" + score + "分。建议关注情绪变化，适当进行心理调适。";
            case "中度抑郁":
                return "您的抑郁症状处于中度水平，标准分" + score + "分。建议寻求专业心理咨询或心理治疗。";
            case "重度抑郁":
                return "您的抑郁症状处于重度水平，标准分" + score + "分。建议尽快寻求专业精神科医生的帮助。";
            default:
                return "您的抑郁症状处于" + depressionLevel + "水平，标准分" + score + "分。";
        }
    }
}