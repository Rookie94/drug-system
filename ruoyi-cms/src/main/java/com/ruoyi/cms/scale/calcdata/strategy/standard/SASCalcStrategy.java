...
import com.ruoyi.cms.scale.calcdata.service.impl.SASScaleCalculator;
...
@Component
public class SASCalcStrategy implements ICalcStrategy {

    private static final Logger logger = LoggerFactory.getLogger(SASCalcStrategy.class);

    @Autowired
    private ILbsContextsService lbsContextsService;

    @Autowired
    private SASScaleCalculator sasScaleCalculator;
    ...
    public ProcResult calculate(ContextAnswerVo contextAnswerVo) {
        ...
        try {
            ...
            SASCalcParams sasCalcParams;
            try {
                sasCalcParams = JSON.parseObject(jsonStr, SASCalcParams.class);
            } catch (Exception e) {
                procResult.setMsg("SAS计算参数解析失败: " + e.getMessage());
                logger.error("SAS计算参数解析失败, json: {}, error: {}", jsonStr, e.getMessage());
                return procResult;
            }

            if (sasCalcParams == null) {
                procResult.setMsg("SAS计算参数解析后为空");
                logger.error("SAS计算参数解析后为空");
                return procResult;
            }

            // 5. 检查计算器是否注入
            if (sasScaleCalculator == null) {
                procResult.setMsg("SASScaleCalculator未正确注入");
                logger.error("SASScaleCalculator未正确注入");
                return procResult;
            }
            ...
            // 7. 调用计算器进行计算
            SASResultDTO sasResult;
            try {
                sasResult = sasScaleCalculator.calculateResult(
                        contextAnswerVo,
                        sasCalcParams,
                        contextVo,
                        null
                ...
                procResult.setMsg("SAS计算过程异常: " + e.getMessage());
                logger.error("SAS计算过程异常, error: ", e);
                return procResult;
            }

            if (sasResult == null) {
                procResult.setMsg("SAS计算结果为空");
                logger.error("SAS计算结果为空");
                return procResult;
            }
            ...
            String resultJson;
            try {
                resultJson = sasScaleCalculator.convertToJson(sasResult);
            } catch (Exception e) {
                procResult.setMsg("SAS结果JSON序列化失败: " + e.getMessage());
                logger.error("SAS结果JSON序列化失败, error: ", e);
                return procResult;
            }

            if (resultJson == null || resultJson.trim().isEmpty()) {
                procResult.setMsg("SAS结果JSON为空");
                logger.error("SAS结果JSON为空");
                return procResult;
            }

            // 9. 设置返回结果
            procResult.setResult(true);
            procResult.setMsg("SAS计算成功");
            procResult.setData(resultJson);

            logger.info("SAS计算成功, contextId: {}, 标准分: {}, 焦虑程度: {}",
                    contextId, sasResult.getStandardScore(), sasResult.getAnxietyLevel());

        } catch (Exception ex) {
            logger.error("SAS计算发生未预期异常: ", ex);
            procResult.setMsg("SAS计算发生未预期异常: " + ex.getMessage());
        }
