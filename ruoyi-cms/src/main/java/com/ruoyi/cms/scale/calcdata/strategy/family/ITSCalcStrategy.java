...
import com.ruoyi.cms.scale.calcdata.service.impl.ITSScaleCalculator;
...
@Component
public class ITSCalcStrategy implements ICalcStrategy {

    private static final Logger logger = LoggerFactory.getLogger(ITSCalcStrategy.class);

    @Autowired
    private ILbsContextsService lbsContextsService;

    @Autowired
    private ITSScaleCalculator itsScaleCalculator;
    ...
    public ProcResult calculate(ContextAnswerVo contextAnswerVo) {
        ...
        try {
            ...
            ITSCalcParams itsCalcParams;
            try {
                itsCalcParams = JSON.parseObject(jsonStr, ITSCalcParams.class);
            } catch (Exception e) {
                procResult.setMsg("ITS计算参数解析失败: " + e.getMessage());
                logger.error("ITS计算参数解析失败, json: {}, error: {}", jsonStr, e.getMessage());
                return procResult;
            }

            if (itsCalcParams == null) {
                procResult.setMsg("ITS计算参数解析后为空");
                logger.error("ITS计算参数解析后为空");
                return procResult;
            }

            // 5. 检查计算器是否注入
            if (itsScaleCalculator == null) {
                procResult.setMsg("ITSScaleCalculator未正确注入");
                logger.error("ITSScaleCalculator未正确注入");
                return procResult;
            }
            ...
            // 7. 调用计算器进行计算
            ITSResultDTO itsResult;
            try {
                itsResult = itsScaleCalculator.calculateResult(
                        contextAnswerVo,
                        itsCalcParams,
                        contextVo,
                        null
                ...
                procResult.setMsg("ITS计算过程异常: " + e.getMessage());
                logger.error("ITS计算过程异常, error: ", e);
                return procResult;
            }

            if (itsResult == null) {
                procResult.setMsg("ITS计算结果为空");
                logger.error("ITS计算结果为空");
                return procResult;
            }
            ...
            String resultJson;
            try {
                resultJson = itsScaleCalculator.convertToJson(itsResult);
            } catch (Exception e) {
                procResult.setMsg("ITS结果JSON序列化失败: " + e.getMessage());
                logger.error("ITS结果JSON序列化失败, error: ", e);
                return procResult;
            }

            if (resultJson == null || resultJson.trim().isEmpty()) {
                procResult.setMsg("ITS结果JSON为空");
                logger.error("ITS结果JSON为空");
                return procResult;
            }

            // 9. 设置返回结果
            procResult.setResult(true);
            procResult.setMsg("ITS计算成功");
            procResult.setData(resultJson);

            logger.info("ITS计算成功, contextId: {}, 总分: {}, 信任程度: {}",
                    contextId, itsResult.getRawTotalScore(), itsResult.getTrustLevel());

        } catch (Exception ex) {
            logger.error("ITS计算发生未预期异常: ", ex);
            procResult.setMsg("ITS计算发生未预期异常: " + ex.getMessage());
        }
