...
import com.ruoyi.cms.scale.calcdata.service.impl.SDSScaleCalculator;
...
@Component
public class SDSCalcStrategy implements ICalcStrategy {

    private static final Logger logger = LoggerFactory.getLogger(SDSCalcStrategy.class);

    @Autowired
    private ILbsContextsService lbsContextsService;

    @Autowired
    private SDSScaleCalculator sdsScaleCalculator;
    ...
    public ProcResult calculate(ContextAnswerVo contextAnswerVo) {
        ...
        try {
            ...
            SDSCalcParams sdsCalcParams;
            try {
                sdsCalcParams = JSON.parseObject(jsonStr, SDSCalcParams.class);
            } catch (Exception e) {
                procResult.setMsg("SDS计算参数解析失败: " + e.getMessage());
                logger.error("SDS计算参数解析失败, json: {}, error: {}", jsonStr, e.getMessage());
                return procResult;
            }

            if (sdsCalcParams == null) {
                procResult.setMsg("SDS计算参数解析后为空");
                logger.error("SDS计算参数解析后为空");
                return procResult;
            }

            // 5. 检查SDSScaleCalculator是否注入
            if (sdsScaleCalculator == null) {
                procResult.setMsg("SDSScaleCalculator未正确注入");
                logger.error("SDSScaleCalculator未正确注入");
                return procResult;
            }
            ...
            // 7. 调用SDSScaleCalculator进行计算（传入ContextVo）
            SDSResultDTO sdsResult;
            try {
                sdsResult = sdsScaleCalculator.calculateResult(
                        contextAnswerVo,
                        sdsCalcParams,
                        contextVo,
                        null  // 这里不传递LbsResultsVo，因为可能还未生成
                );
            } catch (Exception e) {
                procResult.setMsg("SDS计算过程异常: " + e.getMessage());
                logger.error("SDS计算过程异常, error: ", e);
                return procResult;
            }

            if (sdsResult == null) {
                procResult.setMsg("SDS计算结果为空");
                logger.error("SDS计算结果为空");
                return procResult;
            }
            ...
            String resultJson;
            try {
                resultJson = sdsScaleCalculator.convertToJson(sdsResult);
            } catch (Exception e) {
                procResult.setMsg("SDS结果JSON序列化失败: " + e.getMessage());
                logger.error("SDS结果JSON序列化失败, error: ", e);
                return procResult;
            }

            if (resultJson == null || resultJson.trim().isEmpty()) {
                procResult.setMsg("SDS结果JSON为空");
                logger.error("SDS结果JSON为空");
                return procResult;
            }

            // 9. 设置返回结果
            procResult.setResult(true);
            procResult.setMsg("SDS计算成功");
            procResult.setData(resultJson);

            logger.info("SDS计算成功, contextId: {}, 标准分: {}", contextId, sdsResult.getStandardScore());

        } catch (Exception ex) {
            logger.error("SDS计算发生未预期异常: ", ex);
            procResult.setMsg("SDS计算发生未预期异常: " + ex.getMessage());
        }
